...
from PySide6.QtCore import QFile, QObject, Signal, QTimer
...
class PPG(QWidget):
    ...
    def load_ui(self, args_parser):
        ...
        self.record_update_timer = QTimer(self)
        self.record_update_timer.timeout.connect(self.update_time_elapsed)
        self.record_stop_timer = QTimer(self)
        self.record_stop_timer.timeout.connect(self.stop_record_from_timer)
    ...
    def start_recording(self, start_signal):
        ...
        if start_signal:
            ...
            self.record_update_timer.start(1000)
            ...
            if self.ui.timed_acquisition:
                self.record_stop_timer.start(self.ui.curr_acquisition_time_ms)
    ...
    def stop_record_from_timer(self):
        self.ui.data_record_flag = False
        stop_record_thread = threading.Thread(name='stop_record', target=self.stop_record_process, daemon=True)
        stop_record_thread.start()
