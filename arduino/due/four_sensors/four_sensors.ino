...
unsigned int dVal = 0;        // holds the incoming raw data. Signal value can range from 0-4096
...
// Also account for the average processing delay, and substract from the value, e.g. for 250 sampling rate, 4 - 0.65
// 0.05 -> ADC, 0.1 - serial.Print, 0.05 - Misc
int inter_sample_interval_us = int(3.35 * 1000);
...
void loop() {
    ...
    SerialUSB.print(",");
    SerialUSB.print(cVal);        // Send the cVal value to Serial.
    SerialUSB.print(",");
    SerialUSB.println(dVal);      // Send the dVal value to Serial.
    noInterrupts();
